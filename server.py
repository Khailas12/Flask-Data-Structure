...
# This helps to configure the the db collection, configure the sqlite3 to enforce foregin key constraints
...
def create_user():
    ...
    db.session.commit()
    # 200 => status for response, means that the request was successful
...
def get_all_users_descending():
    ...
    # 200 -> to set the deadline for the maximum number of users, or else it'll go infinite
...
def create_blog_post(user_id):
    ...
    user = User.query.filter_by(id=user_id).first()
    if not user:
        return jsonify({"message": "User does not exist"}), 400  # 400 -> Bad response, The client sent a bad request and gotta send it with an existing one 
    ...
    db.session.commit()
    return jsonify({"message": "New Blog Post created"}), 200 # 200 -> success response
